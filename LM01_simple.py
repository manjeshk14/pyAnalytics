...
#S1- libraries
import numpy as np
from sklearn.linear_model import LinearRegression # library imported

#did not use panda because we have the data given in form of array. if we want to import any data then we can use panda.

#S2 - data
x = np.array([5,15,25,35,45,55]).reshape((-1,1)) # reshaping is done to make sure the data enters that way  #making 2 dim
x  #IV
...
y
x, y = np.array(x), np.array(y) #assigning x any to np.array
...
x
model3 = sm.OLS(y,x) # ordinary least squared
model3
results = model3.fit()
results
results.summary()
